...
void	ft_list_remove_if(t_list **begin_list, void *data_ref, int (*cmp)())
{
	...
	while (lst)
	{
		...
		{
			/* if we delete the first element, move the head pointer */
			if (lst == *begin_list)
				*begin_list = lst->next;
			tmp = lst;
			lst = lst->next;
			/* if we delete a subsequent element, move the 'previous' pointer */
			if (prev)
				prev->next = lst;
			free(tmp);
		}
		...
		{
			/* if we don't delete anything, move both pointers */
			prev = lst;
			lst = lst->next;
		}
	}
